...
void main() {
	...
	vec3 lightPos_view  = vec3 (0.0, 0.0, -5.0); // half jupi
    ...
    //float t3 = snoise((position + 3200.0) * 2.0) - s;
     ...
     float threshold = max(t1 * t2, 0.0)*5.0; // threshold determined the place
	 
	 // make them noise
	 float n3 = snoise(position*0.001) * threshold * 5.0; // magic numver frequency
	 

     float n = (n1 + n2 + n3) * 3.14;      //rad
	 
	vec2 rotatedCoord = rotate2D(st.x, st.y, n); // noise rotate/twist themselves
	vec2 newTexCoord = traslate2D(rotatedCoord.x, rotatedCoord.y, n); //then translate themselves
	 
	//color from texture
	vec3 color = texture (normal_map, newTexCoord).rgb * NdotL*Ld; // use diffuse Kd from texture
