...
void main() {
	...
	vec3 lightPos_world  = vec3 (0.0, 0.0, 20.0);
	//vec3 Ld = vec3 (0.5, 0.5, 0.5); // dull diffuse light colour:roughness of surface
	...
	vec3 position = normal_view + vec3(bd, 0.0, bd);
	 ...
	 //------------------- multi octave noise based on snoise------------------
	  //-------------------+ridged-------------------??? function?--------------------------
	...
	frag_colour = vec4(color, 1.0); // add a red/brown-ish color mostly for red spot
