...
void main() {
    ...
    float t3 = snoise((position + 3200.0) * 2.0) - s;
	 float t4 = snoise((position + 800.0) * 1.0) - s;

     // Intersect them and get rid of negatives
     float threshold = max(t1 * t2 * t3 * t4, 0.0)*5.0; // threshold determined the place
	 
	 // make them noise
	 float n3 = snoise(position)*0.02; // magic numver frequency
	 

     float n = n1 + n2 + n3; 
	 
	 //translate first
	 vec2 transCoord = traslate2D(st.x, st.y, n);
	 
	 //then rotate
	vec2 newTexCoord = rotate2D(transCoord.x, transCoord.y, n*3.14); // noise rotate/twist themselves
